...
# Make the phase function for even asphere polynomials.#给出偶非球面多项式的相位函数
# num_coeffs = 8
...
def initialize_params(args):
    ...
    # Number of optimizable phase coefficients可优化的相位系数的数目
    params['num_coeffs'] = args.num_coeffs
    params['phase_func'] = make_phase_func(params['num_coeffs'])  # 输出对应项数的公式
    ...
    params['sensor_pixel'] = 9.4E-6*4  # Meters
    ...
    params['theta_base'] = theta_base  # 0.0,5.0,10.0,15.0
    params['phi_base'] = [0.0]
    ...
    #print('Screen fwhm: {}'.format(params['fwhm']))
    print('sigma: {}'.format(params['sigma']))
    print('pixels_aperture: {}'.format(params['pixels_aperture']))
    print('pixelsX: {}'.format(params['pixelsX']))
    print('pixelsY: {}'.format(params['pixelsY']))
    print('upsample: {}'.format(params['upsample']))
    ...
    print('wavelength_nominal: {}'.format(params['wavelength_nominal']))
    print('pitch: {}'.format(params['pitch']))
    print('Lx: {}'.format(params['Lx']))
    print('Ly: {}'.format(params['Ly']))
    print('xa.shape:',xa.shape,"  ya.shape:",ya.shape)
    print('x_mesh.shape: {}'.format(params['x_mesh'].shape))
    print('y_mesh.shape: {}'.format(params['y_mesh'].shape))

    # Wavelengths and field angles.
    lam0 = params['nanometers'] * tf.convert_to_tensor(np.repeat(lambda_base, np.size(theta_base) * np.size(phi_base)),
                                                    dtype=tf.float32)
    lam0 = lam0[:, tf.newaxis, tf.newaxis]
    lam0 = tf.tile(lam0, multiples=(1, pixelsX, pixelsY))
    params['lam0'] = lam0

    theta = params['degrees'] * tf.convert_to_tensor(np.tile(theta_base, np.size(lambda_base) * np.size(phi_base)),
                                                    dtype=tf.float32)
    ...
    print('lam0.shape: {}'.format(params['lam0'].shape))
    print('degrees: {}'.format(params['degrees']))
    print('theta.shape: {}'.format(params['theta'].shape))
    print('phi.shape: {}'.format(params['phi'].shape))

    # Propagation parameters.
    params['propagator'] = make_propagator(params)
    params['input'] = define_input_fields(params)

    #print('propagator: {}'.format(params['propagator']))
    #print('input: {}'.format(params['input']))
    ...
    # Use a predefined phase pattern (cubic, log-asphere, shifted axicon)使用预定义的相位图(立方，对数非球面，移位轴锥)
    params['use_general_phase'] = args.use_general_phase
    print('use_general_phase: {}'.format(params['use_general_phase']))
...
def make_propagator(params):
    ...
    k = tf.cast(k, dtype=tf.complex64)  # 转换为tf.complex64类型
...
# Propagate the specified fields to the sensor plane.将指定的fields传播到传感器平面。
def propagate(field, params):
    # Field has dimensions of (batchSize, pixelsX, pixelsY)
    # Each element corresponds to the zero order planewave component on the output每个元素对应于输出的零阶平面波分量
    propagator = params['propagator']

    # Zero pad `field` to be a stack of 2n-1 x 2n-1 matrices
    # Put batch parameter last for padding then transpose back把批处理参数最后填充，然后转置回来
    _, _, m = field.shape
    n = params['upsample'] * m
...
# Defines the input electric fields for the given wavelengths and field angles.定义给定波长和视场角下的输入电场
def define_input_fields(params):
    # Define the cartesian cross section定义笛卡儿横截面
    pixelsX = params['pixelsX']#1429
    pixelsY = params['pixelsY']#1429
    dx = params['Lx']  # grid resolution along x 沿x方向的网格分辨率 350E-9
...
# Generates a phase distribution modelling a metasurface given some phase coefficients.生成一个相位分布模型给定一些相位系数的超表面。
def metasurface_phase_generator(phase_coeffs, params):
    ...
    if params['use_proxy_phase'] == True:
        # Determine the duty cycle distribution first.确定占空比分布
        duty = duty_cycle_from_phase(phase_def, params)

        # Accounts for global and local process variations in grating duty cycle.考虑了光栅占空比的全局和局部过程变化。
        if params['fab_tolerancing'] == True:
            ...
            local_error = tf.random.normal(shape=tf.shape(duty), mean=0.0, stddev=params['fab_error_local'],
                                        dtype=tf.float32)
            duty = duty + global_error + local_error

            # Duty cycle is fit to this range and querying outside is not physically meaningful so we need to clip it.占空比适合这个范围，在外面查询是没有物理意义的，所以我们需要剪辑它
    ...
    #print('phase_def',phase_def.shape)
    return phase_def


# Calculates the required duty cycle distribution at the nominal wavelength given在给定的标称波长上计算所需的占空比分布
# a specified phase function using a pre-fit polynomial proxy for the mapping.使用预拟合多项式代理映射的指定相位函数
...
# simulation wavelengths using a pre-fit polynomial proxy for the mapping.使用映射的预拟合多项式代理计算所有所需模拟波长上直径分布的相移。
...
# Finds the intensity at the sensor given the input fields.在给定输入域的情况下，查找传感器处的强度。
...
# Determines the PSF from the intensity at the sensor, accounting for image magnification.根据传感器处的强度确定点扩展函数(PSF)，以说明图像的放大程度。
...
def randomize_wavelengths(params, lambda_base, sigma):
    ...
    lam0 = params['nanometers'] * tf.convert_to_tensor(np.repeat(lambdas, np.size(thetas) * np.size(phis)),
                                                    dtype=tf.float32)
    lam0 = lam0[:, tf.newaxis, tf.newaxis]
    lam0 = tf.tile(lam0, multiples=(1, pixelsX, pixelsY))
    params['lam0'] = lam0
...
def set_wavelengths(params, lambda_base):
    lam0 = params['nanometers'] * tf.convert_to_tensor(
        np.repeat(lambda_base, np.size(params['theta_base']) * np.size(params['phi_base'])), dtype=tf.float32)
    lam0 = lam0[:, tf.newaxis, tf.newaxis]
...
# for the specified wavelength.计算具有双曲面透镜项的立方相位掩模的相位，假设聚焦于指定的波长。
...
# Calculates the phase for a hyperboloidal lens term assuming focusing for the specified wavelength.假设聚焦于指定波长，计算双曲面透镜项的相位。
