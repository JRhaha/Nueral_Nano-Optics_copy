...
def FP(params, args):
    ...
    # otf_4x = tf.keras.layers.Input(shape=[3,int(np.ceil(h/4)),int(np.ceil(w/4))], dtype=tf.complex64)
    # ew_4x  = tf.keras.layers.Input(shape=[int(np.ceil(h/4)),int(np.ceil(w/4)),3])
    ## Feature Extractor
    ...
    side = int(np.ceil((h - params['out_width']) / 4)) * 2
    ...
    # deconv2 = deconv2[:,side//4:side//4+int(np.ceil(side/2)),side//4:side//4+int(np.ceil(side/2)),:]
